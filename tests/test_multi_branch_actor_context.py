...
def test_branch_user_preserves_identity_but_can_select_analytical_primary_branch() -> None:
    ...
    assert actor.can_select_primary_branch is True


def test_locally_assigned_user_can_build_scenario_for_different_primary_branch() -> None:
    user = CurrentUser("u-1", "کاربر شعبه", ("branch_user",), branch_id="101")
    actor = _actor_context(user)
    ...
    assert scenario.actor_context.assigned_branch_code == "101"
    assert scenario.actor_context.actor_scope is ActorScope.BRANCH
